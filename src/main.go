...
func main() {
	...
	if err := driver.Start(); err != nil {
		exit(1, fmt.Sprintf("failed to start browser: %v", err))
	}
	page, err := driver.NewPage()
	if err != nil {
		exit(1, fmt.Sprintf("failed to create browser page: %v", err))
	}
	...
	if !opts.Force {
		...
		if err != nil {
			exit(1, fmt.Sprintf("failed to check working day: %v", err))
		}
		if !isWorkingDay {
			exit(0, fmt.Sprintf("action was skipped (not working day: %s = %s)", today.Format("2006-01-02"), status))
		}
	}
	...
	if err != nil {
		exit(1, fmt.Sprintf("failed to dakoku: %v)", err))
	}

	exit(0, fmt.Sprintf("dakoku was successful => %s", latestAction))
}
